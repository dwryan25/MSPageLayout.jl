...
function comparePage(newpg)
    score::PageScore
    orig = pairsDimensions(pairlist, dse)
    xvals = orig[5]
    yvals = orig[6]
    #Final function will need a margin for error as the optimized layout cannot be expected to match exactly with the original
